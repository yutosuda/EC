...
export class PaymentController {
   ...
   * @param next 次のミドルウェア
   */
  static async createPaymentIntent(req: Request, res: Response, next: NextFunction): Promise<Response | void> {
    try {
   ...
   * @param next 次のミドルウェア
   */
  static async checkPaymentStatus(req: Request, res: Response, next: NextFunction): Promise<Response | void> {
    try {
...
async function handlePaymentSuccess(paymentIntent: any) {
  ...
  if (orderId) {
    ...
    if (order && order.status === OrderStatus.PENDING) {
      order.status = OrderStatus.PAID;
      await order.save();
      console.log(`注文 ${order.id} の決済が完了しました`);
    }
  }
}
...
async function handlePaymentFailure(paymentIntent: any) {
  ...
  if (orderId) {
    ...
    if (order) {
      ...
      await order.save();
      console.log(`注文 ${order.id} の決済が失敗しました: ${paymentIntent.last_payment_error?.message || '不明なエラー'}`);
    }
  }
} 