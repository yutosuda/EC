...
export class WishlistController {
  // お気に入りリストを取得
  getWishlist = async (req: Request, res: Response): Promise<Response | void> => {
    try {
  ...
  // 商品をお気に入りに追加
  addToWishlist = async (req: Request, res: Response): Promise<Response | void> => {
    try {
      const userId = req.user._id;
  ...
  // お気に入りから商品を削除
  removeFromWishlist = async (req: Request, res: Response): Promise<Response | void> => {
    try {
      const userId = req.user._id;
  ...
  // お気に入りをすべて削除
  clearWishlist = async (req: Request, res: Response): Promise<Response | void> => {
    try {
      const userId = req.user._id;
  ...
  // 商品がお気に入りに入っているか確認
  checkProductInWishlist = async (req: Request, res: Response): Promise<Response | void> => {
    try {
      const userId = req.user._id;
      const { productId } = req.params;
