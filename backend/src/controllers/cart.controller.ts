...
export class CartController {
  // カートを取得
  getCart = async (req: Request, res: Response): Promise<Response | void> => {
    try {
  ...
  // 商品をカートに追加
  addToCart = async (req: Request, res: Response): Promise<Response | void> => {
    try {
      const userId = req.user._id;
      ...
      // 合計金額を再計算
      cart.totalAmount = cart.items.reduce((total: number, item: CartItem) => {
  ...
  // カートから商品を削除
  removeFromCart = async (req: Request, res: Response): Promise<Response | void> => {
    try {
      const userId = req.user._id;
  ...
  // カートを空にする
  clearCart = async (req: Request, res: Response): Promise<Response | void> => {
    try {
      const userId = req.user._id;
  ...
  // カート内の商品数量を更新
  updateCartItemQuantity = async (req: Request, res: Response): Promise<Response | void> => {
    try {
      const userId = req.user._id;
      const { productId } = req.params;
